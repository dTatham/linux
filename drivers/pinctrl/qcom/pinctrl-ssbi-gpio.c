					   ...
					   unsigned *num_maps)
{
	...
	if (!ret)
		cfgs[ncfgs++] = pinconf_to_config_packed(PM8XXX_QCOM_DRIVE_STRENGH, val);

	ret = of_property_read_u32(np, "qcom,pull-up-strength", &val);
	if (!ret)
		cfgs[ncfgs++] = pinconf_to_config_packed(PM8XXX_QCOM_PULL_UP_STRENGTH, val);
				 ...
				 unsigned long *config)
{
	...
	case PIN_CONFIG_BIAS_PULL_UP:
		arg = pin->bias <= PM8XXX_GPIO_PULL_UP_1P5_30;
		break;
